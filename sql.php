...
if (isset($_REQUEST['set_col_prefs']) && $_REQUEST['set_col_prefs'] == true) {
    ...
    if (isset($_REQUEST['col_order'])) {
        ...
        $retval = $pmatable->setUiProp(PMA_Table::PROP_COLUMN_ORDER, $col_order, 
    ...
    if ($retval === true && isset($_REQUEST['col_visib'])) {
        ...
        $retval = $pmatable->setUiProp(PMA_Table::PROP_COLUMN_VISIB, $col_visib, 
            $_REQUEST['table_create_time']);
        if (gettype($retval) != 'boolean') {
            PMA_ajaxResponse($retval->getString(), false);
        }
...
if (empty($sql_query) && strlen($table) && strlen($db)) {
    ...
    if (! empty($book_sql_query)) {
        $GLOBALS['using_bookmark_message'] = PMA_message::notice(__(
...
if (isset($find_real_end) && $find_real_end) {
    ...
    $_SESSION['tmp_user_values']['pos'] = @((ceil($unlim_num_rows 
 ...
 */
if (isset($store_bkm)) {
    PMA_Bookmark_save($fields, (isset($bkm_all_users) 
            && $bkm_all_users == 'true' 
        ? true 
        : false));
...
if ($do_confirm) {
    ...
    if ($is_drop_database) {
        echo '<h1 class="error">' . __(
            'You are about to DESTROY a complete database!') . '</h1>';
    }
