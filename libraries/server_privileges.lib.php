 ...
 * @param string $user_host_condition a where clause that containd 
                                      user's host condition
 * @param string $dbname              database name
 *
...
function PMA_getFieldsetForAddDeleteUser($conditional_class)
{
    ...
    $html_output .= '<fieldset id="fieldset_delete_user_footer" ' 
        . 'class="tblFooters">' . "\n";
...
function PMA_getHtmlHeaderForDisplayUserProperties(
    ...
    if (isset($dbname)) {
        ...
        $html_output .= ' - ' 
            . ($dbname_is_wildcard ? __('Databases') : __('Database') );
        ...
        if (! $GLOBALS['is_ajax_request'] 
            ...
            $flushnote = new PMA_Message(
                __(
                    ...
                    . 'from MySQL\'s privilege tables. The content of ' 
                    . 'these tables may differ from the privileges the server ' 
                    . 'uses, if they have been changed manually. In this case, ' 
 ...
 *
 * @param string $user_host_condition user host condition to select 
                                      relevent table privileges
...
function PMA_getTablePrivsQueriesForChangeOrCopyUser($user_host_condition,
    ...
    while ($row = $GLOBALS['dbi']->fetchAssoc($res)) {
        ...
        if (count($tmp_privs2['References']) > 0 
            ...
            $tmp_privs1[] = 'REFERENCES (`' 
                . join('`, `', $tmp_privs2['References']) . '`)';
        }
