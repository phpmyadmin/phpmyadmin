...
class PMA_Scripts
{
    ...
    private function _includeFiles($files)
    {
        ...
        foreach ($files as $value) {
            if (/*overload*/mb_strpos($value['filename'], "?") !== false) {
                if ($value['before_statics'] === true) {
                    $first_dynamic_scripts
                        .= "<script data-cfasync='true' type='text/javascript' src='js/"
                        . $value['filename'] . "'></script>";
                } else {
                    $dynamic_scripts .= "<script data-cfasync='true' type='text/javascript' src='js/"
                        . $value['filename'] . "'></script>";
                }
                continue;
            }
        ...
        $static_scripts = sprintf(
            '<script data-cfasync="true" type="text/javascript" src="%s"></script>',
    ...
    public function getDisplay()
    {
        ...
        $retval .= '<script data-cfasync="true" type="text/javascript">';
        $retval .= "// <![CDATA[\n";
        $retval .= $this->_code;
