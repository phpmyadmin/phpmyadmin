...
function designerMoveLine0(x, y1, y2, foreignKey) {
  ...
  var x1 = x[0];
  var x2 = x[1];
  DesignerMove.line0(x1 + osnTab.offsetLeft, y1 - osnTab.offsetTop, x2 + osnTab.offsetLeft, y2 - osnTab.offsetTop, DesignerMove.getColorByTarget(foreignKey.tableName + '.' + foreignKey.colName));
}
/**
...
DesignerMove.addTableToTablesList = function (index, tableDom) {
  ...
  var $newTableLine = $('<tr>' + '    <td title="' + Messages.strStructure + '"' + '        width="1px"' + '        class="L_butt2_1">' + '        <img alt=""' + '            db="' + dbEncoded + '"' + '            table_name="' + tableEncoded + '"' + '            class="scroll_tab_struct"' + '            src="' + themeImagePath + 'designer/exec.png"/>' + '    </td>' + '    <td width="1px">' + '        <input class="scroll_tab_checkbox"' + '            title="' + checkboxStatus + '"' + '            id="check_vis_' + uniqueId + '"' + '            style="margin:0;"' + '            type="checkbox"' + '            value="check_visible_' + uniqueId + '"' + tableIsChecked + '            />' + '    </td>' + '    <td class="designer_Tabs"' + '        unique_id="' + uniqueId + '">' + $('<div/>').text(db + '.' + table).html() + '</td>' + '</tr>');
  ...
  buttonOptions[Messages.strGo] = function () {
    ...
    if ($table.length !== 0) {
      ...
      if (tableObj && $table.parents()[2].style.display === 'none') {
        $table.parents()[2].style.display = 'block'; // To enable table to be saved to page
  ...
  if (designerTablesEnabled || forceString) {
    ...
    for (key in jTabs) {
      if (document.getElementById('check_vis_' + key).checked) {
        ...
        var tbCoords = new DesignerObjects.TableCoordinate(atob($(designerTable).attr('db_name_base64')), atob($(designerTable).attr('table_name_base64')), -1, x, y);
...
DesignerMove.clickField = function (parentUniqueId, uniqueId, dbBase64, tableBase64, fieldBase64, pk) {
  ...
  if (onDisplayField) {
    ...
    var newDisplayFieldClass = 'tab_field'; // Remove the selected field

    $(document.getElementById('id_tbody_' + parentUniqueId)).find('.tab_field_3,.tab_field_2').attr('class', 'tab_field'); // if is display field

    if (displayField[parentUniqueId] === fieldBase64) {
      ...
      if (displayField[parentUniqueId]) {
  ...
  for (var contrId in contr) {
    ...
    for (var foreignKeyId in scriptContr.foreignKeys) {
      ...
      if (!selected && localX > x1 - 10 && localX < x1 + 10 && localY > y1 - 7 && localY < y1 + 7) {
        DesignerMove.line0(x1 + osnTab.offsetLeft, y1 - osnTab.offsetTop, x2 + osnTab.offsetLeft, y2 - osnTab.offsetTop, 'rgb(255,0,0)');
...
DesignerMove.noHaveConstr = function (idThis) {
  var a = []; // Select all tables and their foreign tables
...
DesignerMove.clickOption = function (dbName, tableName, columnName, tableDbNameUrl, optionColNameString) {
  ...
  designerOptions.style.left = left + 'px'; // var top = GlobY - designerOptions.offsetHeight - 10;
...
DesignerMove.enableTableEvents = function (index, element) {
  ...
  $(element).on('click', '.tab_field_2,.tab_field_3,.tab_field', function () {
    DesignerMove.clickField($(this).attr('parent_uuid'), $(this).attr('unique_id'), $(this).attr('db_base64'), $(this).attr('table_name_base64'), $(this).attr('col_name_base64'), $(this).attr('is_pk_or_unique'));
  });
