  ...
  $(document).on('click', 'input:checkbox.checkall', function (e) {
    ...
    if (!e.shiftKey || lastClickedRow === -1) {
      ...
      if (lastShiftClickedRow >= 0) {
        ...
        $tr.parent().find('tr:not(.noclick)').slice(start, end + 1).removeClass('marked').find(':checkbox').prop('checked', false).trigger('change');
      ...
      $tr.parent().find('tr:not(.noclick)').slice(start, end + 1).addClass('marked').find(':checkbox').prop('checked', true).trigger('change'); // remember the last shift clicked row
...
AJAX.registerOnload('functions.js', function () {
  $(document).on('click', 'a.open_enum_editor', function () {
    ...
    var dialog = '<div id=\'enum_editor\'>' + '<fieldset>' + '<legend>' + title + '</legend>' + '<p>' + Functions.getImage('s_notice') + Messages.enum_hint + '</p>' + '<table class=\'values\'>' + fields + '</table>' + '</fieldset><fieldset class=\'tblFooters\'>' + '<table class=\'add\'><tr><td>' + '<div class=\'slider\'></div>' + '</td><td>' + '<form><div><input type=\'submit\' class=\'add_value btn btn-primary\' value=\'' + Functions.sprintf(Messages.enum_addValue, 1) + '\'></div></form>' + '</td></tr></table>' + '<input type=\'hidden\' value=\'' + // So we know which column's data is being edited
  ...
  $(document).on('click', 'a.central_columns_dialog', function () {
    ...
    var centralColumnsDialog = '<div class=\'max_height_400\'>' + '<fieldset>' + searchIn + '<table id=\'col_list\' class=\'values w-100\'>' + fields + '</table>' + '</fieldset>' + seeMore + '</div>';
...
Functions.toggleButton = function ($obj) {
  ...
  var btnw = $(document.querySelectorAll('table td:nth-child(2)'), $obj).outerWidth(); // Resize the main div so that exactly one side of
...
$(document).on('change', 'input.checkall_box', function () {
  ...
  $(this.form).find(checkboxesSel).not('.row-hidden').prop('checked', isChecked).parents('tr').toggleClass('marked', isChecked);
});
