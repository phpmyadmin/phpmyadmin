...
DesignerMove.resizeOsnTab = function () {
    ...
    for (var key in jTabs) {
        ...
        var kX = parseInt(designerTable.style[mainDirection], 10) + designerTable.offsetWidth;
...
DesignerMove.reload = function () {
    ...
    for (var contrId in contr) {
        ...
        for (var foreignKeyId in scriptContr.foreignKeys) {
            ...
            if (document.getElementById('check_vis_' + foreignKey.tableUuid).checked === false) {
                continue; // if hide
            }
            ...
            DesignerMove.drawLine0(x1, x2, y1, y2, osnTab, foreignKey.tableName + '.' + foreignKey.colName);
        }
    }
};
    ...
    $('#designerModalGoButton').on('click', function () {
        ...
        if ($table.length !== 0) {
            ...
            if (tableObj && $table.parents()[2].style.display === 'none') {
                ...
                document.getElementById('check_vis_' + key).checked = true;
                $(this).dialog('close');
            } else {
    ...
    for (var contrId in contr) {
        ...
        for (var foreignKeyId in scriptContr.foreignKeys) {
            ...
            if (!selected && localX > x1 - 10 && localX < x1 + 10 && localY > y1 - 7 && localY < y1 + 7) {
                DesignerMove.drawLine0(x1, x2, y1, y2, osnTab, 'rgb(255,0,0)');
