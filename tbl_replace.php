...
foreach ($loop_array as $rownumber => $where_clause) {
    ...
    if ($is_insert
        ...
        $prot_row = PMA_DBI_fetch_single_row(
            'SELECT * FROM ' . $common_functions->backquote($table)
            . ' WHERE ' . $where_clause . ';'
        );
    }
    ...
    foreach ($multi_edit_columns_name as $key => $colummn_name) {
        ...
        if ($is_insert) {
            ...
            $query_values[] = $common_functions->backquote(
            ...
            foreach ( $curr_cell_rel_field as $rel_field => $rel_field_value) {
                ...
                $_url_params = array(
                    ...
                    'sql_query' => 'SELECT * FROM '
                        . $common_functions->backquote($map[$rel_field]['foreign_db'])
                        . '.'
                        . $common_functions->backquote($map[$rel_field]['foreign_table'])
                        . ' WHERE '
                        . $common_functions->backquote($map[$rel_field]['foreign_field'])
                        . $where_comparison
                );
