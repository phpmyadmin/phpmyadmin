...
$strInsertIgnoreAsNewRow = '以新行插入 (忽略错误)';
$strInsert = '插入';
$strInterface = '界面';
...
$strJapanese = '日语';
$strJavascriptDisabled = '你的浏览器不支持或禁用了 Javascript，部分 phpMyAdmin 功能将不可用。如导航框架将不能自动刷新。';
$strJoins = '多表查询';
...
$strPdfNoTables = '没有表';
$strPDFPageCreated = '已创建页面';
$strPDFPageCreateFailed = '页面创建失败';
...
$strPmadbCreateConfig = '在配置文件 (<code>config.inc.php</code>) 中启用高级功能，参见 <code>config.sample.inc.php</code> 中的范例。';
$strPmadbCreateHelp = '快速设置高级功能:';
$strPmadbCreateTables = '通过 <code>script/create_tables.sql</code> 创建必需的数据表。';
$strPmadbCreateUser = '创建一个用户并授予其访问上一步操作中创建的数据表的权限。';
$strPmadbReLoginToEnable = '请重新登录 phpMyAdmin 以加载新配置并使其生效。';
...
$strReplicationShowMasterStatus = '查看主服务器状态';
$strReplicationSkippingErrorWarn = '忽略错误可能导致主从服务器间不同步！';
...
$strSetupServers_auth_http_realm_desc = '使用 HTTP 基本认证时显示给用户的提示信息';
$strSetupServers_auth_http_realm_name = 'HTTP 提示信息';
...
$strSQLExportUTC = '以协调世界时 (UTC) 导出时间';
