...
$strIndexes = 'índices';
$strInsecureMySQL = 'Su archivo de configuración contiene parámetros (root sin contraseña) que corresponden a la cuenta privilegiada predeterminada de MySQL. Su servidor de MySQL está corriendo con estos valores, lo que constituye una vulnerabilidad, se le recomienda arreglar esta brecha de seguridad.';
$strInsert = 'Insertar';
...
$strSQL = 'SQL';
$strSQLParserBugMessage = 'Hay una posibilidad de que usted haya encontrado un error en el intérprete de SQL. Por favor examine cuidadosamente su consulta, y verifique que las comillas son usadas adecuadamente y hacen juego. Otra posible causa del fallo es que usted este subiendo un archivo con datos binarios por fuera del área de texto delimitada. Usted puede intentar su consulta en la interfaz de comandos de MySQL. La salida generada por el servidor de MySQL, si la hay aparece abajo, en cuyo caso puede ayudar a diagnosticar el problema. Si aún tiene problemas o el interprete falla en tanto que en la interfaz de comandos funciona, por favor reduzca la salida de su consulta de SQL a la consulta que genera el problema, y envíe un reporte de error con la cadena de datos en la sección de CORTE indicada abajo:';
$strSQLParserUserError = 'Parece haber un error en su consulta de SQL. La salida generada por el servidor de MySQL, si la hay aparece abajo, en cuyo caso puede ayudar a diagnosticar el problema.';
