...
$strAccessDeniedExplanation = 'MySQL サーバに接続しようとしましたが、接続が拒絶されました。config.inc.php の中のホスト、ユーザ名およびパスワードがMySQL サーバの管理者から与えられた情報と一致するか確認してください。';
$strAction = '操作';
...
$strCreateUserDatabaseName = '同名のデータベースを作成してすべての特権を与えます';
$strCreateUserDatabaseNone = 'なし';
$strCreateUserDatabase = 'ユーザ専用データベース';
$strCreateUserDatabaseWildcard = 'ワイルドカードで指定されたデータベースにすべての特権を与えます(ユーザ名_%)';
...
$strDrop = '削除';
$strDropUsersDb = 'ユーザと同じ名前を持つデータベースを削除する';
...
$strEscapeWildcards = 'ワイルドカード「_」および「%」を文字通りに使用するときは「\\」でエスケープしてください。';
$strEsperanto = '国際語';
$strEstonian = 'エストニア語';
...
$strField = 'フィールド';
$strFileAlreadyExists = 'ファイル「%s」は既にサーバに存在しします。ファイル名を変更するか、上書きオプションを確認してください。';
...
$strInvalidDatabase = 'データベースが不正です';
...
$strInvalidServerHostname = 'サーバ「%1$s」のホスト名が無効です。設定を調査してください。';
...
$strNoDataReceived = 'インポートするデータを受信できませんでした。ファイル名が送信されていないか、ファイルサイズがPHPの設定で許可された最大値を超えています。FAQ 1.16をご覧ください';
...
$strNoPermission = 'ファイル「%s」を保存するパーミッションをウェブサーバは持っていません。';
$strNoPhp = 'PHP コード無し';
...
$strPrivDescCreateUser = 'ユーザアカウントの作成・破棄・名称変更を許可します。';
...
$strRunSQLQueryOnServer = 'サーバ「%s」で SQL を実行する';
$strRussian = 'ロシア語';
...
$strSetEnumVal = 'フィールドタイプが「enum」又は「set」の場合は値にこの書式を使って入力してください: \'a\',\'b\',\'c\'...<br />バックスラッシュ「"\\"」又はシングルクオート「"\'"」を含めたい場合は(「\\\\xyz」や「a\\\'b」のように)バックスラッシュでエスケープしてください';
$strShowAll = 'すべて表示';
$strShowColor = '色表示';
...
$strShowStatusHandler_read_firstDescr = 'インデックスからはじめのエントリを読み込んだ回数です。これが高い場合、それはサーバが多くの完全なインデックス走査を行っていると示します。例えば「SELECT col1 FROM foo」は col1 はインデックスと仮定します。';
